...
async def command_start_handler(message: Message, command: CommandObject) -> None:
    ...
    if (args):
        ...
        if (len(args) == 1 and dest_city in PATHS[location_id]):
            ...
            # connection.close()
            global bot
            distance = abs(LOCATIONS[location_id]['coords'][0] - LOCATIONS[dest_city]['coords'][0]) + abs(LOCATIONS[location_id]['coords'][1] - LOCATIONS[dest_city]['coords'][1])
            ...
            connection = sqlite3.connect('test.db')
            cur = connection.cursor()
            cur.execute("UPDATE Person SET LocationID = ? WHERE UserID = ?", (dest_city, message.from_user.id, ))
            connection.commit()
            connection.close()
            return
